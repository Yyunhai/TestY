...
/**
 * 认证核心服务。
 * 负责多表注册、登录校验、密码重置、角色权限装配以及登录审计记录。
 */
@Service
public class AuthService {
     ...
     */
    @Transactional
    public AuthenticatedUser register(String username, String email, String rawPassword, String displayName, String phoneNumber) {
        ...
        return loadAuthenticatedUser(savedAccount);
    }
     ...
     */
    @Transactional
    public AuthenticatedUser resetPassword(String username, String email, String newPassword) {
        String normalizedUsername = normalizeUsername(username);
        String normalizedEmail = normalizeEmail(email);
